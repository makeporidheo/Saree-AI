...
import { Loader2, TrendingUp, Check, Download, AlertCircle, RefreshCw, Sparkles, ChevronRight, Square, CheckSquare, Settings, LogOut, MessageSquarePlus, User as UserIcon, Image as ImageIcon, Tag, IndianRupee, Copy, HelpCircle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Track which saree is selected for processing
  const [selectedSareeIndex, setSelectedSareeIndex] = useState<number | null>(null);
    ...
    if (updates.newPassword) {
      setUsers(prev => prev.map(u => 
        u.username === currentUser.username 
          ? { ...u, password: updates.newPassword, ...updates } 
          : u
      ));
  ...
  const handleStartAnalysis = useCallback(async () => {
    if (selectedSareeIndex === null) { setStatus(prev => ({ ...prev, error: "Please select one saree to proceed." })); return; }
    if (!formData.modelFace) { setStatus(prev => ({ ...prev, error: "Please upload a model face image." })); return; }
    
    const selectedSaree = formData.productImages[selectedSareeIndex];
    const newProducts: ProductWorkflowItem[] = [{
      id: `prod-${Date.now()}-0`,
      file: selectedSaree,
      step2Selected: true,
      step2Status: 'idle',
    ...
    setStep(AppStep.VisualizeAndAnalyze);
    ...
    try {
        ...
        for (let i = 0; i < newProducts.length; i++) {
            ...
            try {
                ...
                const [generatedImgUrl, analysis] = await Promise.all([
                    generateTryOnImage(sareeBase64, faceBase64, formData.modelFigureDesc, formData.backgroundDesc, formData.attireDesc),
                    analyzeSareePotential(sareeBase64, formData.attireDesc)
                ]);
        ...
        setCompletedSteps(prev => [...new Set([...prev, AppStep.Upload, AppStep.VisualizeAndAnalyze])]);
    } catch (err: any) {
  ...
  }, [formData, selectedSareeIndex, completedSteps]);
  ...
  const handleRegenerateTryOn = async (productId: string) => {
    ...
    try {
        ...
        const generatedImgUrl = await generateTryOnImage(sareeBase64, faceBase64, formData.modelFigureDesc, formData.backgroundDesc, formData.attireDesc);
  ...
  const handleProceedToProductShots = async () => {
    ...
    if (selectedProducts.length === 0) { alert("Please select a product to proceed."); return; }
            ...
            for (const prod of itemsToProcess) {
                try {
                    ...
                    for (let i = 0; i < shotConfigs.length; i++) {
                        ...
                        try {
                             const url = await generateSingleProductShot(sareeBase64, faceBase64, prod.tryOnImage || null, shotConfig, { figureDesc: formData.modelFigureDesc, bgDesc: formData.backgroundDesc, attireDesc: formData.attireDesc });
    ...
    setCompletedSteps(prev => [...new Set([...prev, AppStep.EcommerceImages])]);
  };
  ...
  const handleRegenerateShot = async (productId: string, shotId: string) => {
      ...
      try {
          const sareeBase64 = await fileToBase64(product.file);
          const faceBase64 = formData.modelFace ? await fileToBase64(formData.modelFace) : null;
          const url = await generateSingleProductShot(sareeBase64, faceBase64, product.tryOnImage || null, { ...shot, feedback: shot.isRefining ? shot.feedback : undefined }, { figureDesc: formData.modelFigureDesc, bgDesc: formData.backgroundDesc, attireDesc: formData.attireDesc });
  ...
  const handleProceedToDetails = () => {
      if (products.filter(p => p.step3Selected && p.step3Status === 'success').length === 0) { alert("Please select a product to proceed."); return; }
      setStep(AppStep.ProductDetails);
      setCompletedSteps(prev => [...new Set([...prev, AppStep.ProductDetails])]);
  }
  ...
  const handleGenerateSEO = async (productId: string) => {
      ...
      try {
          const sareeBase64 = await fileToBase64(product.file);
          const seoData = await generateSEODetails(sareeBase64, product.details.fabric, product.details.design, formData.attireDesc);
  ...
  const handleDownloadZip = async () => {
    ...
    if (selectedProducts.length === 0) { alert("Please select a product to download."); return; }
    ...
    try {
        ...
        for (const product of selectedProducts) {
            ...
            // Original Image (convert to blob)
            const origRes = await fetch(URL.createObjectURL(product.file));
            const origBlob = await origRes.blob();
            folder.file(getNextImageName(), origBlob);
  ...
  const renderStep1 = () => (
    <div className="max-w-5xl mx-auto">
      <div className="bg-white rounded-2xl shadow-sm p-8 border border-slate-100">
        <h2 className="text-2xl font-serif font-bold text-primary-900 mb-6 flex items-center gap-2">
          Product & Model Details
          <div className="group relative">
             <HelpCircle size={16} className="text-slate-400 cursor-help" />
             <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-800 text-white text-[10px] rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 text-center font-sans font-normal">
               Upload up to 5 sarees. Select exactly one to proceed to visualization.
             </div>
          </div>
        </h2>
        <div className="grid md:grid-cols-2 gap-8">
            <ImageUploadField 
              label="Saree Product Images (Up to 5)" 
              subLabel="Upload up to 5 variants. Select one to proceed." 
              files={formData.productImages} 
              multiple={true} 
              maxFiles={MAX_PRODUCT_IMAGES} 
              onFilesSelected={(newFiles) => {
                const updated = [...formData.productImages, ...newFiles];
                updateFormData('productImages', updated);
                if (selectedSareeIndex === null && updated.length > 0) setSelectedSareeIndex(0);
              }} 
              onRemoveFile={(index) => {
                const updated = formData.productImages.filter((_, i) => i !== index);
                updateFormData('productImages', updated);
                if (selectedSareeIndex === index) setSelectedSareeIndex(updated.length > 0 ? 0 : null);
                else if (selectedSareeIndex !== null && selectedSareeIndex > index) setSelectedSareeIndex(selectedSareeIndex - 1);
              }} 
              selectedIndex={selectedSareeIndex}
              onSelect={(index) => setSelectedSareeIndex(index)}
            />
            <ImageUploadField 
              label="Model Face" 
              subLabel="Upload a reference face for try-on." 
              files={formData.modelFace ? [formData.modelFace] : []} 
              maxFiles={1} 
              onFilesSelected={(files) => updateFormData('modelFace', files[0])} 
              onRemoveFile={() => updateFormData('modelFace', null)} 
            />
        </div>
        <div className="grid md:grid-cols-2 gap-6 mb-8 mt-6">
          ...
          <div><label className="block text-sm font-semibold text-slate-700 mb-2">Background Setting</label><textarea value={formData.backgroundDesc} onChange={(e) => updateFormData('backgroundDesc', e.target.value)} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none h-32 resize-none" placeholder="Describe setting..." /></div>
          <div className="md:col-span-2"><label className="block text-sm font-semibold text-slate-700 mb-2">Saree & Attire Details</label><textarea value={formData.attireDesc} onChange={(e) => updateFormData('attireDesc', e.target.value)} className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none h-32 resize-none" placeholder="Fabric details..." /></div>
        </div>
        <button 
          onClick={handleStartAnalysis} 
          disabled={formData.productImages.length === 0 || !formData.modelFace || selectedSareeIndex === null} 
          className="w-full py-4 bg-primary-700 hover:bg-primary-900 text-white rounded-xl font-bold text-lg shadow-lg flex justify-center items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <span>Generate Visualization & Analytics</span>
          <TrendingUp size={20} />
        </button>
            ...
            {products.map((item) => (
                <div key={item.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden transition-all">
                    <div className="bg-slate-50 px-6 py-3 border-b flex justify-between items-center">
                        <div className="flex items-center gap-3"><span className="font-semibold text-slate-700">Analysis Result</span>{item.step2Status === 'loading' && <span className="text-xs text-primary-600 flex items-center"><Loader2 size={12} className="animate-spin mr-1"/> Generating...</span>}</div>
                        {item.analysis && <span className="text-sm bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-medium">Rating: {item.analysis.rating}/10</span>}
                    </div>
                    <div className="p-6">
  ...
  const renderStep3 = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto pb-20 animate-fade-in">
             <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-serif font-bold text-slate-900">Product Showcase (9 Shots)</h2></div>
            <div className="space-y-16">
                {activeProducts.map((item) => (
                    <div key={item.id} className="border-b pb-12">
                        <div className="flex items-center gap-4 mb-6"><h3 className="text-xl font-serif font-bold text-slate-800">Gallery</h3><p className="text-sm text-slate-500">{item.ecommerceImages?.filter(i => i.status === 'success').length || 0}/9 Ready</p></div>
            ...
            <div className="space-y-8">
                {activeProducts.map((item) => (
                    <div key={item.id} className="bg-white rounded-xl shadow-sm border overflow-hidden">
                        <div className="flex flex-col sm:sm:items-center justify-between gap-4 p-6 border-b bg-slate-50">
                            <div className="flex items-center gap-4"><img src={item.tryOnImage} className="w-12 h-16 object-cover rounded bg-white border" /><div className="font-serif font-bold">Product Details</div></div>
                        </div>
                        <div className="p-6">
  ...
  const renderStep5 = () => {
      ...
      return (
        <div className="max-w-4xl mx-auto text-center py-10 animate-fade-in"><div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6"><Check size={40} /></div><h2 className="text-3xl font-serif font-bold text-slate-900 mb-4">Project Completed</h2><p className="text-slate-600 mb-8">Catalog generated successfully.</p>
            <div className="grid gap-4 text-left">
                {finalProducts.map((p, i) => (
                    <div key={p.id} className="bg-white p-4 rounded-xl shadow-sm border flex gap-4 cursor-pointer border-green-500 ring-1"><img src={p.tryOnImage || URL.createObjectURL(p.file)} className="w-20 h-28 object-cover rounded bg-slate-100" /><div className="flex-1"><div><h3 className="font-bold text-slate-800">{p.details.seo?.seoTitle || `Saree Product`}</h3><span className="text-[10px] bg-slate-100 px-2 py-1 rounded font-mono">{p.details.seo?.sku || 'NO-SKU'}</span></div><div className="grid grid-cols-4 gap-2 text-xs text-slate-500 mt-2"><div><span className="block font-bold">Price</span>₹{p.details.salePrice}</div><div><span className="block font-bold">Vendor</span>{p.details.vendorCode || '-'}</div><div><span className="block font-bold">Fabric</span>{p.details.fabric || '-'}</div></div></div></div>
                ))}
            </div>
            <div className="flex justify-center gap-4 mt-10"><button onClick={handleDownloadZip} disabled={status.isGenerating} className="px-6 py-3 bg-slate-900 text-white rounded-lg font-semibold flex items-center gap-2 disabled:opacity-50">{status.isGenerating ? <Loader2 className="animate-spin" size={20} /> : <Download size={20} />}Download Catalog Zip</button><button onClick={() => window.location.reload()} className="px-6 py-3 bg-primary-600 text-white rounded-lg font-semibold">Start New Project</button></div>
        </div>
      )
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} vendors={vendors} setVendors={setVendors} users={users} setUsers={setUsers} />
