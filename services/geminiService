...
/**
 * Retrieves the Gemini API Client.
 * It checks localStorage for a user-provided key first, then falls back to process.env.API_KEY.
 * This satisfies the request for flexibility outside of the AI Studio environment.
 */
const getClient = () => {
  ...
  if (!apiKey) {
    throw new Error("API Key not found. Please set it in the Admin Console or environment variables.");
  }
